...
const SubmitBtnArea = cc('div', {children:[
  m(CreateBtn).text('Create').on('click', async () => {
    try {
      ...
      util.ajax({method:'POST',url:'/api/create-island',alerts:Alerts,buttonID:CreateBtn.id,body:body},
        (id) => {
          islandID = id;
  ...
  m(UpdateBtn).text('Update').hide().on('click', async () => {
    try {
      const body = await newIslandForm();
      util.ajax({method:'POST',url:'/api/update-island',alerts:Alerts,buttonID:UpdateBtn.id,body:body},
...
async function init() {
  ...
  if (islandID) {
    ...
    util.ajax({method:'GET',url:'/api/get-island',alerts:Alerts,body:body},
      (resp) => {
